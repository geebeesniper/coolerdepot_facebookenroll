<?php
/**
 * File / 文件：scripts/migrate_v0_1_55.php
 * EN: CLI maintenance/deployment script for migrate v0 1 55.
 * 中文：用于 migrate v0 1 55 的命令行维护/部署脚本。
...
$before = $pdo->prepare(
    "SELECT external_post_id,published_at,published_date
...
$after = $pdo->prepare(
    "SELECT external_post_id,title,published_at,published_date
     FROM cdsp_sales_posts
     WHERE platform='facebook'
       AND external_post_id IN ($placeholders)
     ORDER BY external_post_id"
...
echo "v0.1.55 post-date correction complete." . PHP_EOL;
