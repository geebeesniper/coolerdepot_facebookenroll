 ...
 * and before provider/fetch verification; Bulk Submit Post is a sibling workflow.
 */
...
$checks['Dashboard exposes Bulk Submit Post beside Submit Post']=str_contains($dashboard,'data-sales-i18n="submitPost"')&&str_contains($dashboard,'data-sales-i18n="bulkSubmitPost"')&&str_contains($dashboard,'/sales/bulk-submit');
$checks['Dedicated Bulk Submit Post page exists']=str_contains($bulk,'Bulk Submit Post')&&str_contains($bulk,'id="salesBulkUrls"')&&str_contains($bulk,'id="bulkQueueButton"');
$checks['Bulk Submit page provides CSRF for queue actions']=str_contains($bulk,'name="_csrf"')&&str_contains($bulk,'id="salesBulkCsrf"');
$checks['Bulk Submit button keeps the Bulk Submit Post label after requests']=str_contains($js,"text(salesTr('bulkSubmitPost'))");
$checks['Bulk Submit route exists']=str_contains($routes,"'/sales/bulk-submit'")&&str_contains($routes,"'bulkSubmitForm'");
$checks['Bulk Submit controller action exists']=str_contains($controller,'function bulkSubmitForm');
$checks['Bulk Submit is a peer navigation item']=str_contains($header,"\$navActive['bulk_submit']")&&str_contains($header,'data-nav-i18n="bulkSubmit"');
...
$checks['No Bulk Edit label exists']=stripos($submit.$dashboard.$bulk.$header.$js,'Bulk Edit')===false;
