...
class Inspection {
    public static function create(array $d):string{
        ...
        $s=Database::connection()->prepare("INSERT INTO post_inspections
    ...
    public static function verified(string $token,int $uid):?array{
        $s=Database::connection()->prepare("SELECT * FROM post_inspections WHERE token=? AND sales_user_id=? AND verification_status='verified' AND consumed_at IS NULL AND expires_at>=NOW() LIMIT 1");
        $s->execute([$token,$uid]);return$s->fetch()?:null;
    }
    public static function consume(int $id):void{$s=Database::connection()->prepare("UPDATE post_inspections SET consumed_at=NOW() WHERE id=?");$s->execute([$id]);}
}
