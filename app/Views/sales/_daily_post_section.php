<?php
/**
 * File / 文件：app/Views/sales/_daily_post_section.php
 * EN: Renders the sales/_daily_post_section application view template.
 * 中文：渲染应用视图模板 sales/_daily_post_section。
...
>
    ...
    <div class="sales-post-card-grid">
        <?php if (empty($day['posts'])): ?>
            <div
                class="sales-empty-message sales-day-empty-message"
                role="status"
            >
        ...
        <?php foreach ($day['posts'] as $index => $p): ?>
            ...
            <article
                ...
                data-sales-post-account-id="<?= Util::e((string)($p['platform_account_id']??'')) ?>"
                data-sales-post-account-name="<?= Util::e((string)($p['platform_account_name']??'')) ?>"
                data-sales-post-account-url="<?= Util::e((string)($p['platform_account_url']??'')) ?>"
                role="button"
                tabindex="0"
